...
// refresh — once they've scraped the bare homepage they keep serving that card.
...
export function buildShareUrl(token: number): string {
  return `${SITE_URL}/?s=${token.toString(36)}`;
}
